...
public interface SimpleHibernateDao<T> {
	 ...
	 * @return 查询结果
	 */
	public List<T> findBy(final String propertyName, final Object value, final String propertyName1,
			final Object value1);
	 ...
	 * @return 查询结果
	 */
	public List<T> findByAndOrder(final String propertyName, final Object value, final String orderBy,
			final boolean isAsc);
	 ...
	 * @return
	 */
	public List<T> findByAndOrder(final String propertyName, final Object value, final String propertyName1,
			final Object value1, final String orderBy, final boolean isAsc);
	 ...
	 * 按属性查找唯一对象,匹配方式为相等.
	 */
	public T findUniqueBy(final String propertyName, final Object value, final String propertyName1,
			final Object value1);
	...
	/**
	 * 根据查询HQL与参数列表创建Query对象. Create Query object based on the query of hql and
	 * attribute array
	 * 
	 * 本类封装的find()函数全部默认返回对象类型为T,当不为T时使用本函数. All the packaged find() function
	 * will return a default object type T. Use function itself when it is not T.
	 * 
	 * @param values
	 *            amount changeable attribute, binding by order
	 * @return
	 */
